...
public class Crocodoc {
   ...
   * 
   * @param string
   *          path The path on the server to make the request to relative to the
   *          base path
   * @param string
   ...
   * @throws CrocodocException
   */
  private static HttpResponse _requestHttpResponse(String path, String method,
    ...
    try {
      ...
      if (postParams != null && postParams.size() > 0) {
        ...
        for (Map.Entry<String, Object> param : postParams.entrySet()) {
          if (param.getValue().getClass().equals(FileBody.class)) {
            mpEntity.addPart(param.getKey(), (FileBody) param.getValue());
          } else {
            StringBody stringBody = new StringBody(param.getValue().toString(),
   ...
   * @param object
   *          response This is a representation of the response, usually from
   *          JSON, but can also be a string
   * 
   * @throws CrocodocException
   */
