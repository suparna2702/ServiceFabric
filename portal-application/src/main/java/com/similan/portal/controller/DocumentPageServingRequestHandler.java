...
@Controller
@RequestMapping(value = "/documentpage", method = RequestMethod.GET)
@Slf4j
public class DocumentPageServingRequestHandler {
	 ...
	 */
	@RequestMapping(value = "/thumbnail/{workspaceOwnerName}/{workspaceName}/{documentName}/{documentInstanceVersion}/{number}", method = RequestMethod.GET)
	public void handle(
			...
			@PathVariable("number") Integer number, HttpServletResponse response)
			throws ServletException, IOException {

    DocumentPageKey pageKey = new DocumentPageKey(URLDecoder.decode(
        workspaceOwnerName, "ISO-8859-1"),
        URLDecoder.decode(workspaceName, "ISO-8859-1"), URLDecoder.decode(
            documentName, "ISO-8859-1"), documentInstanceVersion, number);
		AssetStream stream = storage.retrievePage(pageKey);
		if (stream == null) {
		...
		log.info("Asset with key " + pageKey + " served successfully");
	}
}
